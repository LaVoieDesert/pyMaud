...
def get_parser(ligne, debug=False):
    if ligne.startswith("data_"):
        if debug: print("parsed data_: {}".format(ligne))
        return parse_datablock
    elif ligne.startswith("loop_"):
        if debug: print("parsed loop_: {}".format(ligne))
        return parse_cifloop
    elif ligne.startswith("#subordinateObject_"):
        if debug: print("parsed subordinateObject_: {}".format(ligne))
        return parse_subordinateobject
    elif ligne.startswith("#custom_object_"):
        if debug: print("parsed custom_object_: {}".format(ligne))
        return parse_custom_object
    elif ligne.startswith("_"):
        if debug: print("parsed value_: {}".format(ligne))
        return parse_value
    else:
        if debug: print("not parsed: {}".format(ligne))
        return None
